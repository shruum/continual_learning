...
class ERRM(ContinualModel):
    NAME = 'errm'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ERRM, self).__init__(backbone, loss, args, transform)
        ...
        if self.current_task > 0:
            for name, param in self.net.named_parameters():
                if name in self.layer_weights: #change
                    ...
                    mask = self.masks[ind].repeat(param.shape[0], 1,1,1).to(self.device)
                    param.grad *= mask
    ...
    def end_task(self, dataset) -> None: #
        ...
        if self.current_task < (dataset.N_TASKS - 1):
            ...
            rein_masks = [torch.ones(3, 3).repeat(self.layer_sizes[i], 1, 1) for i in range(len(self.layer_sizes))] 
            ...
            all_neurons = torch.topk(std_vals, self.args.top_neurons_rein, largest=False)[1].detach().cpu() # to reinitialize
            ...
            for class_id in range(dataset.N_CLASSES_PER_TASK):
                ...
                top5_ind = torch.topk(vals[task_ind], min(task_ind.shape[0], self.args.top_neurons))[1].detach().cpu().numpy()
            ...
            for i, sz in enumerate(self.n_neurons):
                ...
                rein_masks[i][neurons, :, :] = 0 #torch.zeros(3, 3)
            ...
            for name, param in self.net.named_parameters():
                if name in self.layer_weights: #change
                    ind = self.layer_weights.index(name)
                    mask = rein_masks[ind].repeat(param.shape[0], 1,1,1).to(self.device)
