...
lst_reg_weight = [0.15, 0.25, 0.5, 1]
lst_mixup_alpha = [1, 4, ]
lst_feat_level = [4]
lst_buffer_size = [500]
num_runs = 3
    ...
    for buffer_size in lst_buffer_size:
        for feat_level in lst_feat_level:
            for reg_weight in lst_reg_weight:
                for mixup_alpha in lst_mixup_alpha:

                        train_params = best_params[buffer_size]

                        exp_id = f"clserpp-c10-{buffer_size}-param-{train_params['idt']}-r-{reg_weight}-f-{feat_level}-m-{mixup_alpha}-s-{seed}"
                        job_args = ["-c", f"python /git/continual_learning/mammoth/main.py  \
                            ...
                            --output_dir /output/cil_er_mixup_cifar10 \
                            --tensorboard \
                            --csv_log \
                            "]
                        # set job params
