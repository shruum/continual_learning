...
def save_model(args, model, model_save_dir, task, epoch=0):
    ...
    for model_name in lst_models:
        if model_name == 'net':
            model_save_dir_t = os.path.join(os.path.dirname(model_save_dir), args.experiment_id)
        else:
            model_save_dir_t = os.path.join(os.path.dirname(model_save_dir), args.experiment_id + f'_{model_name}')
